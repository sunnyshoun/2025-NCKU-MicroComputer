...
# 1 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include/pic18.inc" 1 3





# 1 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include/pic18_chip_select.inc" 1 3
# 770 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include/pic18_chip_select.inc" 3
# 1 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include\\proc/pic18f4520.inc" 1 3
# 47 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include\\proc/pic18f4520.inc" 3
PORTA equ 0F80h
...
TOSU equ 0FFFh
# 4698 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include\\proc/pic18f4520.inc" 3
...
# 771 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include/pic18_chip_select.inc" 2 3
# 7 "C:\\Program Files\\Microchip\\xc8\\v3.00\\pic\\include/pic18.inc" 2 3
...
global BSS, BSH, COUNT, BIT_CNT, W_TEMP, STATUS_TEMP, BSR_TEMP, DELAY_CT
BSS: DS 1
BSH: DS 1
COUNT: DS 1 ; C 傳入 192 (BUF_SIZE)
BIT_CNT: DS 1
W_TEMP: DS 1 ; 在 C code 中也用作 BYTE_TEMP
STATUS_TEMP: DS 1
BSR_TEMP: DS 1
DELAY_CT: DS 1
...
_output_grb:
    ; 保存上下文
    MOVWF W_TEMP, 0 ; W_TEMP 暫存 WREG (內含 COUNT LSB)
    MOVFF STATUS, STATUS_TEMP
    MOVFF BSR, BSR_TEMP

    ; 禁用中断
    BCF INTCON, 7, 0 ; ((INTCON) and 0FFh), 7, a = 0

    ; 加载 COUNT (來自 W_TEMP)
    MOVF W_TEMP, 0, 0
    MOVWF COUNT, 0 ; C 傳入 192 (BUF_SIZE)

send_byte_loop:
    ; 加载 1 字节数据
    MOVF POSTINC0, 0, 0 ; W = *FSR0, 且 FSR0++
    MOVWF W_TEMP, 0 ; 將 "目前要傳送的 byte" 存入 W_TEMP

    MOVLW 0x08 ; 8 bits
    MOVWF BIT_CNT, 0

send_bit_loop:
    RLNCF W_TEMP, 1, 0 ; 1 週期. 旋轉 W_TEMP (MSB -> Carry)
    BSF LATB, 0, 0 ; 1 週期. Pin High
    BTFSC STATUS, 0, 0 ; 1 (C=1) 或 2 (C=0)
    BRA bit_1_path

; 0-bit path (C=0)
    ; T0H = 1(RLNCF) + 1(BSF) + 2(BTFSC) = 4 週期 (400ns) - GOOD
    BCF LATB, 0, 0 ; 1 週期. Pin Low
    NOP ; 1
    NOP ; 1
    NOP ; 1
    ; T_low_part1 = 1 + 3 = 4 週期
    BRA bit_join

bit_1_path:
    ; T1H = 1(RLNCF) + 1(BSF) + 1(BTFSC) + 2(BRA) = 5 週期
    NOP ; 1
    NOP ; 1
    NOP ; 1
    ; T1H = 5 + 3 = 8 週期 (800ns) - GOOD
    BCF LATB, 0, 0 ; 1 週期. Pin Low
    ; T_low_part1 = 1 週期
    ; (直接落到 bit_join)

bit_join:
    ; T_low_part2: 迴圈 overhead
    DECF BIT_CNT, 1, 0 ; 1 週期
    BNZ send_bit_loop ; 2 週期 (如果跳轉)

    ; --- 迴圈時序分析 (Bits 7...1) ---
    ; T0L = 4 (part1) + 1(DECF) + 2(BNZ) = 7 週期 (700ns)
    ; T1L = 1 (part1) + 1(DECF) + 2(BNZ) = 4 週期 (400ns)
    ; *註: T0L 700ns 略低於 850ns，但在 ±150ns 容許範圍內 (700-1000ns)，OK!
    ; *註: T1L 400ns 也OK (300-600ns)。

    ; --- 第 8 位元 (Bit 0) 的時序 ---
    ; T_low_part2 (Bit 0): DECF(1) + BNZ(1, 不跳轉) = 2 週期

    DECF COUNT, 1, 0 ; 1 週期
    BNZ send_byte_loop ; 2 週期 (如果跳轉)
    ; T_low_part3 (位元組間) = 1 + 2 = 3 週期

    ; 總 T0L (Bit 0) = 4(part1) + 2(part2) + 3(part3) = 9 週期 (900ns) - OK!
    ; 總 T1L (Bit 0) = 1(part1) + 2(part2) + 3(part3) = 6 週期 (600ns) - OK!
...
delay_loop:
    ...
    ; 恢复中断和上下文
    BSF INTCON, 7, 0 ; ((INTCON) and 0FFh), 7, a = 1
    MOVFF BSR_TEMP, BSR
    MOVFF STATUS_TEMP, STATUS
    MOVF W_TEMP, 0, 0 ; 恢复 W (雖然 C 函式不一定需要)
